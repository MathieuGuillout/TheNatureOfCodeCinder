#include "cinder/gl/gl.h"
#include "cinder/Rand.h"

#include "Box.h"
#include "Convert.h"

using namespace ci;
Box::Box(b2World * _world, Rectf _rect, bool isMoving, Color _color) {
  angle = 0;
  world = _world;
  color = _color;
  rect = _rect;

  b2BodyDef bd;
  b2Vec2 p = Convert::toPhysics(rect.getCenter());

  bd.position.Set(p.x, p.y);
  bd.type = isMoving ? b2_dynamicBody : b2_staticBody;
 
  body = world->CreateBody(&bd);

  b2PolygonShape ps;
  ps.SetAsBox(
    Convert::toPhysics(rect.getWidth() / 2), 
    Convert::toPhysics(rect.getHeight() / 2)
  );

  b2FixtureDef fixtureDef;
  fixtureDef.shape = &ps;
  fixtureDef.density = 10.0f;
  fixtureDef.friction = 0.8f;
  fixtureDef.restitution = 0.1f;
  body->CreateFixture(&fixtureDef);

  body->SetUserData(this);
}


void Box::update() {
  Vec2f position = Convert::toScreen(body->GetPosition());
  position -= rect.getSize() / 2;
  rect = Rectf(position, position + rect.getSize());
  angle = body->GetAngle();
}

void Box::change() {
  color = Color(randFloat(1), randFloat(1), randFloat(1));
}

void Box::draw() {
  gl::color(this->color);

  gl::pushMatrices();

  gl::translate(rect.getCenter());
  gl::rotate(angle);
  gl::drawSolidRect((rect - rect.getUpperLeft()) - rect.getSize() / 2);

  gl::popMatrices();
}

void Box::killBody() {
  world->DestroyBody(body);
}
